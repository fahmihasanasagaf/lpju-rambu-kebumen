<aside x-data="{ open: false }" class="fixed inset-y-0 left-0 z-40 flex w-72 -translate-x-full flex-col bg-navy-900 text-white transition-transform duration-200 lg:translate-x-0" :class="{ 'translate-x-0': open }" aria-label="Navigasi utama">
    ...
    <nav class="flex-1 space-y-1 px-3 py-5">
        ...
        <a href="{{ route('assets.lpju.index') }}" class="flex min-h-11 items-center gap-3 rounded-lg border-l-4 border-transparent px-3 text-sm text-slate-300 transition-colors hover:bg-navy-800 hover:text-white">
            <svg class="h-5 w-5" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="m12 3 2.1 5.1L19 10l-4.9 1.9L12 17l-2.1-5.1L5 10l4.9-1.9L12 3Z" /></svg>
            Data aset
        </a>
        <a href="{{ route('assets.rambu.index') }}" class="flex min-h-11 items-center gap-3 rounded-lg border-l-4 border-transparent px-3 text-sm text-slate-300 transition-colors hover:bg-navy-800 hover:text-white">
            <svg class="h-5 w-5" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 3v18m0-18 8 4-8 4-8-4 8-4Zm0 8 8 4-8 4-8-4 8-4Z" /></svg>
            Data Rambu
        </a>
...
<div x-data="{ open: false }" class="lg:hidden">
    <button type="button" @click="open = !open" class="fixed left-4 top-4 z-50 flex h-11 w-11 items-center justify-center rounded-lg bg-navy-900 text-white shadow-lg" aria-label="Buka navigasi">
